...
#define PIN_CS1    D0  
#define PIN_DRDY1  D1  
#define PIN_RESET  D2  // reset all chips 
#define PIN_CS2    D3  
#define PIN_SCLK   D8  
#define PIN_MISO   D9  
#define PIN_MOSI   D10 
...
//  (safe boot)
BleGamepad bleGamepad("Octopos_16", "PiEEG", 100);
...
void adcAcquisitionTask(void* pvParameters) {
    while (true) {
        //await interrupt from first ADC
        ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
        
        AdcOutput raw1 = adc1.readAdcRaw();
        AdcOutput raw2 = adc2.readAdcRaw();

        // Filling the circular buffer with mutex protection
        ...
        // We transmit a custom raw data packet via BLE if a client is connected
        ...
        //We wake up Core 0 for DSP calculations strictly every 4 samples (16 ms).
...
// CORE 0: ISOLATED MATHEMATICAL CALCULATION
...
// 2. FFT & Normalization
      ...
      // 3. Calculation of the Topological Tensor (ciPLV) — exactly the same as in Dongle
       ...
       // 4. Scaling for the number of pairs
      ...
      // 6. Calculation of raw stick values ​​(1-to-1 as in exportAxes from App.tsx)
    ...
    // SMOOTHING DISABLED (0 ms latency, 1-to-1 responsiveness like in the Python script)
        smooth_lx = raw_lx;
        smooth_ly = raw_ly;
        smooth_rx = raw_rx;

      // Limit the physical range of the sticks strictly to the [-1.0 ... 1.0] interval
...
void setup() {
    ...
    if (id2 == 0x0000 || id2 == 0xFFFF) {
        activeChannels = 8;
        Serial.println("[SYSTEM] ADC2 not found. Operating in 8-channel mode (Octopos_16).");
    } else {
        activeChannels = 16;
        Serial.println("[SYSTEM] ADC2 found. Operating in 16-channel mode (Octopos_16).");
    }
    ...
    // === CRITICAL DELAY FOR STABILIZATION ===
    delay(500);

  // === STEP 5: SAFE AD RECONFIGURATION ===
    NimBLEAdvertising* pAdvertising = NimBLEDevice::getAdvertising();
    if (pAdvertising) {
        pAdvertising->stop();
        delay(100); 

       // Primary Advertising Package (Primary Advertisement)
...
// === CREATING DEDICATED TASKS ON DIFFERENT CORES ===    
  
    // 1. Tarea de captura ADC en el núcleo 1 con alta prioridad 10 (TIEMPO REAL ESTRICTO)
    xTaskCreatePinnedToCore(adcAcquisitionTask, "ADCTask", 8192, NULL, 10, &ADCTaskHandle, 1);
    
  // 2. DSP calculation task on Core 0 with priority 3
...
// ===================== CORE 1: LOW-PRIORITY SYSTEM LOOP =====================
void loop() {
    // loop() is freed from polling DRDY and runs at the lowest priority (1).
    // Slow operations no longer create phase noise for the ADC.
